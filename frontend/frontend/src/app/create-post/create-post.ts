...
export class CreatePost {
    ...
    this.postsService.createPost(payload).subscribe({
      ...
      error: (err) => {
        ...
        if (backendErrors && typeof backendErrors === 'object') {
          this.backendErrors.set(Object.values(backendErrors).map((message) => this.mapBackendError(String(message))));
          this.errorMessage.set(err?.error?.message || 'Bitte korrigiere die markierten Eingaben.');
        } else {
          this.backendErrors.set([]);
          this.errorMessage.set(err?.error?.message || 'Beitrag konnte nicht gespeichert werden. Bitte versuche es erneut.',);
        }
        this.isLoading.set(false);
      },
    });
  ...
  private mapBackendError(message: string) {
    switch (message) {
